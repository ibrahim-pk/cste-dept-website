          ...
          <div className="mx-auto grid grid-cols-1 place-content-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {allTeacher.length > 0 ? (
              allTeacher.map((item, idx) => (
                <div
                  key={idx}
                  className="card teacherCard w-full max-w-md mx-auto sm:mx-0	bg-base-100 shadow-xl hover:shadow-lg"
                >
                  <Link to={`/teacher/details/${item._id}`}>
                    <figure>
                      <img src={item.imgUrl} alt="teacher" className="w-full" />
                    </figure>
